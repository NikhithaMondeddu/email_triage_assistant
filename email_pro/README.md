...
- Long threads (default: ≥10 messages) are sent to ScaleDown; you get a compressed summary so the agent can process full context without token blow-up.
- Set `THREAD_SCALEDOWN_THRESHOLD` and `SCALEDOWN_RATE` in `.env` if needed.
- Without `SCALEDOWN_API_KEY`, triage still runs using the full thread text (no compression).

## Deliverables

| Deliverable | Location |
|------------|----------|
| Email assistant plugin | `src/assistant.py`, `plugin_cli.py` |
| Productivity metrics | `src/deliverables/productivity_metrics.py`, `data/productivity_metrics.json` |
| User satisfaction surveys | `src/deliverables/satisfaction_surveys.py`, `data/satisfaction_surveys.json` |
| Inbox zero achievement rate | `src/deliverables/inbox_zero.py`, `data/inbox_zero_history.json` |
...
├── plugin_cli.py          # CLI entrypoint
├── requirements.txt
├── .env.example
├── data/                  # credentials, tokens, metrics, surveys (gitignore)
└── src/
    ├── models.py          # EmailMessage, EmailThread, TriageResult, etc.
    ├── scaledown_client.py
    ├── assistant.py       # Main EmailAssistant
    ├── providers/         # Gmail, Outlook
    ├── agents/            # Triage, PriorityScorer, DraftGenerator, FollowUpTracker
    ├── engines/           # RulesEngine
    ├── features/          # SmartFolders, UrgentDetector, MeetingExtractor, UnsubscribeSuggestions
    └── deliverables/     # ProductivityMetrics, SatisfactionSurveys, InboxZeroTracker
```
